#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import json
import logging
import sys
import os
import psycopg2

from datetime import datetime
from config.config import ADDCART_TABLE
from utils.log_utils import setup_logger

logger = setup_logger("addtocart_db_processor", "logs/addtocart_db_processor.log")

def insert_addtocart_event(conn, event):
    """장바구니 이벤트를 Postgres에 적재"""
    if not event:
        logger.warning("빈 이벤트 수신, DB 적재 건너뜀")
        return

    try:
        with conn.cursor() as cur:
            sql = f"""
                INSERT INTO {ADDCART_TABLE} (
                    order_id, user_id, user_name, email, 
                    product_id, product_name, quantity, price, 
                    total, status, created_at
                )
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                ON CONFLICT (order_id) DO NOTHING
            """

            cur.execute(sql, (
                event.get("order_id"),
                event.get("user_id"),
                event.get("user_name"),
                event.get("email"),
                event.get("product_id"),
                event.get("product_name"),
                event.get("quantity"),
                event.get("price"),
                event.get("total"),
                event.get("status"),
                datetime.now()
            ))
            conn.commit()
            logger.info(f"AddToCart 이벤트 적재 완료: {event.get('order_id')}")
    except Exception as e:
        logger.error(f"DB 적재 실패: {e}")

